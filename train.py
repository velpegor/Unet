...
def train(model, trainloader, optimizer, tau_optimizer, criterion, writer, logger, config, epoch, gpu):
...
def main_worker(gpu, ngpus_per_node, config):
    ...
    traindata = dset.VOCSegmentation(root='/home/velpegor/workspace/UNet-pytorch/dataset', 
                                     year='2012', 
                                     image_set='train', 
                                     transforms=train_transform, download=True)
    validdata = dset.VOCSegmentation(root='/home/velpegor/workspace/UNet-pytorch/dataset', 
                                     year='2012', 
                                     image_set='val', 
                                     transforms=val_transform, download=True)
    ...
    tau_params = [param for name, param in model.named_parameters() if 'tau' in name]
    # 'tau'가 아닌 파라미터를 추출
    other_params = [param for name, param in model.named_parameters() if 'tau' not in name]

    # 각 파라미터 그룹에 대해 별도의 옵티마이저 생성
    tau_optimizer = torch.optim.Adam(tau_params, lr=config.init_lr/10, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-6, amsgrad=False)
    optimizer = torch.optim.Adam(other_params, lr=config.init_lr, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-6, amsgrad=False)

    # 각 옵티마이저에 대해 별도의 스케줄러 생성
    tau_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(tau_optimizer, config.epoch, eta_min=config.last_lr, verbose=True)
    other_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, config.epoch, eta_min=config.last_lr, verbose=True)
    ...
    for epoch in range(1, config.epoch+1):
        ...
        if config.rank % ngpus_per_node == 0:
            logger.info(f'Epoch {epoch} Learning Rate: {other_scheduler.get_lr()[0]}')
        
        dist.barrier()
        train(model, trainloader, optimizer, tau_optimizer, criterion, writer, logger, config, epoch, gpu)
